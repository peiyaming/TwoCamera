...
//			matrix.setRotate(getPreviewDegree(context));  
...
//	    	最后我们只需要使用画布给图片加水印就可以了：
	    ...
	    public static boolean getStatus(String start, String end) {
			try{
				...
				et = end.split(":");
				if("".equals(st[0])||"".equals(st[1])||"".equals(et[0])||"".equals(et[1])) return false;
				// 开始时间
	   ...
	   /* 
	    ...
	    public static  boolean timerTaskForVoice(Context context,String time,final Handler handler) {
	    	boolean b=false;
	        ...
	        if(TimeCompare(date)){
	        	timer.schedule(task,strToDateLong(date));
	        	b=true;
	        	Log.i("TAG", "开始任务");
	        }
			return b;
	    ...
	    private static boolean TimeCompare(String date2){
	        //格式化时间
	...
	public static String changeDex(String ten_str){
		...
		try{
			...
			if((cardLong+"").length()<10){
				cardno = String.format("%010d",cardLong);}else{
				cardno=cardLong+"";
			}
		}catch(Exception e){
			e.printStackTrace();
